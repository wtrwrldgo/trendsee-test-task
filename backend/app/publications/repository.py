...
async def create_publication(user_id: int, title: str, content: str) -> asyncpg.Record:
    pool = get_pool()
...
async def get_publication_by_id(pub_id: int) -> asyncpg.Record | None:
    pool = get_pool()
    return await pool.fetchrow(
        "SELECT id, user_id, title, content, created_at, updated_at FROM publications WHERE id = $1",
        pub_id,
    )
...
async def update_publication(pub_id: int, title: str | None, content: str | None) -> asyncpg.Record | None:
    pool = get_pool()
    ...
    return await pool.fetchrow(
        """UPDATE publications SET title = $1, content = $2, updated_at = NOW()
           WHERE id = $3
           RETURNING id, user_id, title, content, created_at, updated_at""",
        new_title,
        new_content,
